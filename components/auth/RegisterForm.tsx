...
export function RegisterForm() {
  ...
  const handleOAuthRegister = async (provider: 'google' | 'github') => {
    try {
      ...
      Alert.alert(
        t('auth.comingSoon'),
        t(`auth.${provider}OAuthComingSoon`, `${provider} registration will be available soon`),
      );
      ...
      Alert.alert(
        ...
        t(`error.auth.${provider}_oauth_failed`, `${provider} registration failed`),
      );
    } finally {
      ...
      <FormControl isInvalid={!!errors.username} isRequired>
        ...
        <Input className="border-outline-300" size="lg" isInvalid={!!errors.username}>
          <InputField
            ...
            onChangeText={txt => setValue('username', txt, { shouldValidate: true })}
            autoCapitalize="none"
      ...
      {(error || errors.root) && (
        <Box className="rounded-lg border border-error-200 bg-error-50 p-3">
          <Text className="text-center text-sm text-error-700">
            {errors.root?.message || error?.message || t('error.auth.registerFailed')}
          </Text>
        </Box>
      )}
