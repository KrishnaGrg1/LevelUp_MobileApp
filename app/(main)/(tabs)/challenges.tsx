      ...
      <Box className="px-6 py-6">
        ...
        <HStack className="mb-6" space="md">
          ...
          >
            ...
            >
              {t('challenges.completed', 'Completed')} ({completedChallenges.length})
            </Text>
          </Pressable>
        </HStack>
              ...
              <Card className="border-0 bg-white p-4 shadow-sm dark:bg-background-900">
                ...
                <HStack className="items-start justify-between">
                  <VStack className="flex-1">
                    <HStack className="items-center" space="sm">
                      ...
                      <Badge size="sm" variant="outline" className="border-outline-300">
                        <BadgeText className="text-typography-900">{challenge.category}</BadgeText>
                      </Badge>
                    </HStack>
                  ...
                  {challenge.isActive ? (
                    <Box className="h-12 w-12 items-center justify-center rounded-full bg-background-100">
                      <Icon as={TrendingUp} size="lg" className="text-blue-500" />
                    </Box>
                  ) : (
                    <Box className="h-12 w-12 items-center justify-center rounded-full bg-green-500">
                      <Icon as={CheckCircle2} size="lg" className="text-white" />
                    </Box>
                  )}
                </HStack>
                ...
                <Box className="mt-4">
                  ...
                  <Box className="h-2 overflow-hidden rounded-full bg-background-100">
                    <Box
                      className={`h-full ${challenge.isActive ? 'bg-primary-500' : 'bg-green-500'}`}
                      style={{
                ...
                <HStack className="mt-4 items-center justify-between">
                  <HStack space="md">
                    ...
                    <HStack className="items-center" space="xs">
                      ...
                      <Text className="text-sm text-typography-500">{challenge.participants}</Text>
                    </HStack>
                  </HStack>
                  {challenge.isActive ? (
                    <HStack className="items-center" space="xs">
                      <Icon as={Clock} size="sm" className="text-red-500" />
                      <Text className="text-sm font-medium text-red-500">{challenge.timeLeft}</Text>
                    </HStack>
                  ) : (
                    <HStack className="items-center" space="xs">
                      <Icon as={Calendar} size="sm" className="text-typography-500" />
                      <Text className="text-sm text-typography-500">{challenge.completedDate}</Text>
                    </HStack>
                  )}
                </HStack>
              </Card>
