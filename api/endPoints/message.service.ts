...
export const getClanMessages = async (
  ...
  try {
    const response = await axiosInstance.get(
      `/community/clan/${clanId}/messages`,
      {
        params: { page, limit },
        headers: { "X-Language": language },
        withCredentials: true,
    ...
    throw new Error(
      error.response?.data?.body?.message || "Failed to fetch clan messages"
    );
  }
};
...
export const checkClanMembership = async (
  ...
  try {
    const response = await axiosInstance.get(
      `/community/clan/${clanId}/membership/${userId}`,
      {
        headers: { "X-Language": language },
        withCredentials: true,
      }
    ...
    throw new Error(
      error.response?.data?.body?.message || "Failed to check clan membership"
    );
  }
};
